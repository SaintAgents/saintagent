  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="max-w-sm p-0">
        <DialogHeader className="p-4 pb-3 border-b bg-gradient-to-r from-violet-50 to-pink-50 dark:from-violet-900/20 dark:to-pink-900/20">
        ...
        {/* Prompts */}
        <div className="p-4 space-y-2" key={shuffleKey}>
          {displayPrompts.map((prompt, idx) => (
            <button
              key={idx}
              onClick={() => handleSelect(prompt)}
              className="w-full text-left p-3 rounded-lg border border-slate-200 dark:border-slate-700 hover:border-violet-300 dark:hover:border-violet-500 hover:bg-violet-50 dark:hover:bg-violet-900/20 transition-all text-sm text-slate-700 dark:text-slate-300 hover:text-slate-900 dark:hover:text-slate-100"
            >
              "{prompt}"
            </button>
          ))}
        ...
        {/* Shuffle */}
        <div className="p-4 pt-2 border-t bg-slate-50 dark:bg-slate-900/50">
          <Button
            variant="ghost"
            size="sm"
