...
export default function SocialShareButtons({ 
  ...
  const handleShare = (platform) => {
    ...
    // For platforms that need copy to clipboard (Instagram, TikTok)
    if (config.copyToClipboard) {
      const shareText = `${text}\n${url}`;
      navigator.clipboard.writeText(shareText).then(() => {
        toast.success(`Copied to clipboard! Paste in ${config.name}`);
        window.open(config.getShareUrl(url, text), '_blank');
      }).catch(() => {
        window.open(config.getShareUrl(url, text), '_blank');
      });
      return;
