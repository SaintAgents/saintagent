...
export default function RankedAvatar({
  ...
  const rankCode = rpRankCode || getRPRank(rpPoints || 0)?.code || 'seeker';
  const cfg = getRingConfig(rankCode);
  const padPx = cfg.pad; // outer ring thickness in px
