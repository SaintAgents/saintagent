...
import {
  Database, RefreshCcw, Clock, Zap, Shield, Save,
  Trash2, BarChart3, Activity, HardDrive, Settings2,
  AlertTriangle, CheckCircle2, TrendingUp, ArrowLeft
} from 'lucide-react';
...
export default function CacheAdmin() {
  ...
  // Load saved config from PlatformSetting
  ...
  const updateGroup = (key, field, value) => {
    setConfig(prev => ({
      ...
      groups: {
        ...prev.groups,
        [key]: { ...prev.groups[key], [field]: value }
      }
    }));
    setDirty(true);
  };
  ...
  // Determine top query keys by count
        ...
        <div className="max-w-7xl mx-auto px-6 py-8">
          <div className="flex items-center gap-4 mb-4">
            ...
            <div>
              ...
              <p className="text-slate-400 text-sm">Monitor and tune API query caching to prevent rate limiting</p>
            </div>
          </div>
          {dirty && (
            <div className="flex items-center gap-3 mt-4 p-3 bg-amber-500/20 border border-amber-500/30 rounded-lg">
              <AlertTriangle className="w-5 h-5 text-amber-400" />
              <span className="text-amber-200 text-sm">Unsaved changes</span>
              <Button
                size="sm"
        ...
        <Card>
          ...
          <CardContent>
            <div className="flex flex-wrap gap-3">
              <Button variant="outline" className="gap-2" onClick={handleInvalidateAll}>
                <RefreshCcw className="w-4 h-4" /> Invalidate All Caches
              </Button>
              <Button variant="outline" className="gap-2 text-red-600 border-red-200 hover:bg-red-50" onClick={handleFlushAll}>
                  ...
                  {queryKeyGroups.map(([key, data]) => (
                    <tr key={key} className="border-b border-slate-100">
                      ...
                      <td className="py-2 pr-4 text-center">
                        <Badge variant={data.stale > 0 ? 'secondary' : 'outline'} className="text-xs">
                          {data.stale}
                        </Badge>
                      </td>
                      <td className="py-2 pr-4 text-center">
