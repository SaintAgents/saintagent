  ...
  <div className="min-h-screen cmd-deck-bg pb-32 md:pb-20 w-full max-w-full overflow-x-hidden m-0 p-0">
    <div className={cn(
      ...
      <div className="px-0 md:px-6 pt-6 pb-4 w-full" style={{ marginTop: '48px' }}>
              ...
              <PopoverContent className="w-72 p-4" align="end" side="bottom" sideOffset={5} collisionPadding={16}>
                <h4 className="font-semibold text-slate-900 dark:text-white mb-3 flex items-center gap-2">
                  ...
                  <ol className="space-y-2 text-sm">
                    <li className="flex items-start gap-2"><span className="flex-shrink-0 w-5 h-5 rounded-full bg-violet-100 dark:bg-violet-900/50 text-violet-600 dark:text-violet-400 text-xs font-bold flex items-center justify-center">1</span><span className="text-slate-700 dark:text-slate-300">Complete your profile with avatar & bio</span></li>
                    <li className="flex items-start gap-2"><span className="flex-shrink-0 w-5 h-5 rounded-full bg-violet-100 dark:bg-violet-900/50 text-violet-600 dark:text-violet-400 text-xs font-bold flex items-center justify-center">2</span><span className="text-slate-700 dark:text-slate-300">Join a Circle or create your own</span></li>
                    <li className="flex items-start gap-2"><span className="flex-shrink-0 w-5 h-5 rounded-full bg-violet-100 dark:bg-violet-900/50 text-violet-600 dark:text-violet-400 text-xs font-bold flex items-center justify-center">3</span><span className="text-slate-700 dark:text-slate-300">Book your first meeting with a match</span></li>
                    <li className="flex items-start gap-2"><span className="flex-shrink-0 w-5 h-5 rounded-full bg-violet-100 dark:bg-violet-900/50 text-violet-600 dark:text-violet-400 text-xs font-bold flex items-center justify-center">4</span><span className="text-slate-700 dark:text-slate-300">Create an offer or join a mission</span></li>
                    <li className="flex items-start gap-2"><span className="flex-shrink-0 w-5 h-5 rounded-full bg-amber-100 dark:bg-amber-900/50 text-amber-600 dark:text-amber-400 text-xs font-bold flex items-center justify-center">5</span><span className="text-emerald-600 dark:text-emerald-400 font-semibold">📖 Earn GGG! Refer friends, use Advanced view, submit projects.</span></li>
                    <li className="flex items-start gap-2"><span className="flex-shrink-0 w-5 h-5 rounded-full bg-cyan-100 dark:bg-cyan-900/50 text-cyan-600 dark:text-cyan-400 text-xs font-bold flex items-center justify-center">6</span><span className="text-slate-700 dark:text-slate-300">Read the <strong className="text-violet-600 dark:text-violet-400">Learn</strong> card in Activity Cards below</span></li>
                  </ol>
                </PopoverContent>
              </Popover>
            </div>
        ...
        <ControlsDeck colCOrder={colCOrder} setCardsForceOpen={setCardsForceOpen} setHiddenCards={setHiddenCards} setStoredCards={setStoredCards} setColCOrder={setColCOrder} storeAllCards={storeAllCards} restoreAllCards={restoreAllCards} theme={theme} onThemeToggle={onThemeToggle} profile={profile} visibleCards={visibleCards} setCustomCards={setCustomCards} setDeckViewMode={setDeckViewMode} setHiddenCardsFromSuggestions={setHiddenCards} />
        ...
        <div className="px-0 md:px-6 relative min-h-[1200px] w-full max-w-full overflow-x-hidden">
          <div className="block space-y-6">
            ...
            {isCardVisible('communityFeed') && <CollapsibleCard title="Community Feed" cardId="communityFeed" icon={Sparkles} backgroundImage="https://qtrypzzcjebvfcihiynt.supabase.co/storage/v1/object/public/base44-prod/public/694f3e0401b05e6e8a042002/ae589aa03_universal_upscale_0_56f51cb9-0490-420c-a398-fabdc48611df_0.jpg" defaultOpen={true} forceOpen={cardsForceOpen} isHidden={hiddenCards.has('communityFeed')} onToggleHide={() => toggleCardVisibility('communityFeed')} onTossToSidePanel={handleTossToSidePanel} onPopout={() => {}} onRefresh={() => queryClient.refetchQueries({ queryKey: ['communityPosts'] })} onAdd={() => {setQuickCreateType('post');setQuickCreateOpen(true);}}>
              <CommunityFeedCard maxHeight="400px" />
            </CollapsibleCard>}

            {isCardVisible('circles') && <CollapsibleCard title="Circles & Regions" cardId="circles" icon={Users} defaultOpen={false} backgroundImage="https://images.unsplash.com/photo-1620712943543-bcc4688e7485?w=800&q=80" onPopout={() => setCirclesPopupOpen(true)} forceOpen={cardsForceOpen} isHidden={hiddenCards.has('circles')} onToggleHide={() => toggleCardVisibility('circles')} onTossToSidePanel={handleTossToSidePanel} onRefresh={() => queryClient.refetchQueries({ queryKey: ['circles'] })} onAdd={() => window.location.href = createPageUrl('Circles')}>
              <CirclesRegions />
            </CollapsibleCard>}
          ...
          <div className="block space-y-6 mt-6">
            ...
            {isCardVisible('meetings') && <CollapsibleCard title="Meetings & Momentum" cardId="meetings" icon={Calendar} badge={pendingMeetings.length > 0 ? `${pendingMeetings.length} pending` : undefined} badgeColor="amber" backgroundImage="https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?w=800&q=80" onPopout={() => setMeetingsPopupOpen(true)} forceOpen={cardsForceOpen} isHidden={hiddenCards.has('meetings')} onToggleHide={() => toggleCardVisibility('meetings')} onTossToSidePanel={handleTossToSidePanel} navigateTo={createPageUrl('Meetings')} onRefresh={() => queryClient.refetchQueries({ queryKey: ['dashboardMeetings'], exact: true })} onAdd={() => {setQuickCreateType('meeting');setQuickCreateOpen(true);}}>
              <div className="space-y-3">
            ...
            {isCardVisible('missions') && <CollapsibleCard title="Missions" cardId="missions" icon={Target} badge={missions.length} badgeColor="amber" backgroundImage="https://images.unsplash.com/photo-1517486808906-6ca8b3f04846?w=800&q=80" onPopout={() => setMissionsPopupOpen(true)} forceOpen={cardsForceOpen} isHidden={hiddenCards.has('missions')} onToggleHide={() => toggleCardVisibility('missions')} onTossToSidePanel={handleTossToSidePanel} navigateTo={createPageUrl('Missions')} onRefresh={() => queryClient.refetchQueries({ queryKey: ['dashboardMissions', currentUser?.email], exact: true })} onAdd={() => {setQuickCreateType('mission');setQuickCreateOpen(true);}}>
              <div className="space-y-3">
                {missions.length === 0 ?
                <div className="text-center py-6">
                ...
                <div className="space-y-6" ref={provided.innerRef} {...provided.droppableProps}>
                  ...
                  <Draggable draggableId={id} index={index} key={id}>
                    ...
                    <div ref={dragProvided.innerRef} {...dragProvided.draggableProps} {...dragProvided.dragHandleProps}>
                            {id === 'market' &&
                      <CollapsibleCard title="Marketplace: Earn & Learn" cardId="market" icon={ShoppingBag} backgroundImage="https://images.unsplash.com/photo-1639322537228-f710d846310a?w=800&q=80" onPopout={() => setMarketPopupOpen(true)} forceOpen={cardsForceOpen} isHidden={hiddenCards.has('market')} onToggleHide={() => toggleCardVisibility('market')} onTossToSidePanel={handleTossToSidePanel} navigateTo={createPageUrl('Marketplace')} onRefresh={() => queryClient.refetchQueries({ queryKey: ['listings'] })} onAdd={() => {setQuickCreateType('offer');setQuickCreateOpen(true);}}>
